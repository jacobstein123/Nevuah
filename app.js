...
app.post("/createteacher", function(req, res) {
    req_params = ["school", "authkey", "teacher", "email", "passwd"];
    given_params = req.body;
    for (var param in req_params) {
        var param = req_params[param]
        if (param in given_params) {} else {
...
app.post("/createstudent", function(req, res) {
    req_params = ["school", "authkey", "email", "passwd", "name"];
    given_params = req.body;
    for (var param in req_params) {
        var param = req_params[param]
        if (param in given_params) {} else {
...
app.post("/signin", function(req, res) {
    ...
    var user= db.collection("user");
    user.findOne({"email":given_params.email}, function (err, result){
    	console.log(result);
    	if (result[0].passwd==given_params.passwd){
    		res.send({"congradulations!": " You are a horse"})
    	};
    	else{
